...
public class Prob2 {
	...
	public Integer Dijkstra(int source, int target) {
		...
		while (vSource != vTarget) {
			...
			// System.out.println("remove: " + vSource + "|" + shortestPathLength.get(vSource.shortVertex) + "|" + vSource.weight);
			//heap.print();
			...
			for(Edge e : vSource.edges) {
				...
				e.target().shortVertex = vSource.value;
				// System.out.println("processing: " + vSource.value + "|" + vSource.weight + "|" + shortestPathLength.get(vSource.value));
				if (index == -1) {
					// System.out.println("insert " + e.target().value);
					e.target().weight = shortestPathLength.get(vSource.value) + e.weight;
					heap.insert(e.target());
					//heap.print();
				} else {
					// System.out.println("replace " + e.target().value);
		...
		//heap.print();
		return shortestPathLength.get(vTarget.value);
	}
